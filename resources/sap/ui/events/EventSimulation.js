...
sap.ui.define([
	...
	/**
	 * @exports sap/ui/events/EventSimulation
	 * @private
	 */
	 ...
	 * @private
	 */
	oEventSimulation._fnMouseToTouchHandler = function(oEvent, oConfig) {
	...
	oEventSimulation._initTouchEventSimulation = function() {
		this._createSimulatedEvent("touchstart", ["mousedown"], this._fnMouseToTouchHandler);
		this._createSimulatedEvent("touchend", ["mouseup", "mouseout"], this._fnMouseToTouchHandler);
		...
		this._createSimulatedEvent("touchmove", ["mousemove", "dragstart"], this._fnMouseToTouchHandler);
	};
	...
	if (Device.browser.webkit && /Mobile/.test(navigator.userAgent) && Device.support.touch) {
		MapTouchToMouse.init(window.document);
		oEventSimulation.disableTouchToMouseHandling = MapTouchToMouse.disableTouchToMouseHandling;
	}
