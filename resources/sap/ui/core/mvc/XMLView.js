				...
				if ( aChildren ) {
					for (var i = 0; i < aChildren.length; i++) {
						...
						var oChildDOM = aChildren[i].getDomRef() ||
										jQuery.sap.domById(sap.ui.core.RenderPrefixes.Invisible + aChildren[i].getId());


						// if DOM exists, replace the preservation dummy with it
						if ( oChildDOM ) {
							jQuery.sap.byId(sap.ui.core.RenderPrefixes.Dummy + aChildren[i].getId(), this._$oldContent).replaceWith(oChildDOM);
